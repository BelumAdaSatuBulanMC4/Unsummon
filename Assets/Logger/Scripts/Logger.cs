...
namespace EasyUI{
    ...
    public class Logger : MonoBehaviour
    {
        ...
        private bool isOpen = false; // Fixed typo 'isOppen' to 'isOpen'
        ...
        private void ScrollDown(){
            // Check if uiScrollRect is not null before accessing it
            if (uiScrollRect != null) {
                uiScrollRect.verticalNormalizedPosition = 0f;
            } else {
                Debug.LogWarning("ScrollRect has been destroyed or is null.");
            }
        }
        ...
        private void ToggleLogUI(){
            isOpen = !isOpen; // Fixed typo 'isOppen' to 'isOpen'
            if (isOpen)
        ...
        private void SetupUI(Vector2 size, Sprite icon){
            uiScrollRect.enabled = isOpen; // Only enable uiScrollRect if log UI is open
        ...
        private void OnDisable() {
            Application.logMessageReceived -= LogCallback; // Use -= to unsubscribe
